...
private final TileEntityEnergyStorage tileEntity;
	...
	public ContainerEnergyStorage(InventoryPlayer player, TileEntityEnergyStorage storage) {
		tileEntity = storage;
		addSlotToContainer(new SlotBattery(tileEntity, 0, 61, 35));
        ...
        for (int i = 0; i < crafters.size(); ++i){
            ...
            while(t < 6){
            	if(giveEnergyToSide[t] != tileEntity.giveEnergyToSide[t]){
            		icrafting.sendProgressBarUpdate(this, t + 7, tileEntity.giveEnergyToSide[t] ? 1 : 0);
            	}
            	++t;
            }
            if(timer <= 0){
